...
void gfx_open( int width, int height, const char *title )
{
	...
	XSelectInput(gfx_display, gfx_window, StructureNotifyMask|KeyPressMask|ButtonPressMask|KeyReleaseMask|ButtonReleaseMask);
...
int gfx_event_waiting()
{
       ...
       while (1) {
               if(XCheckMaskEvent(gfx_display,-1,&event)) {
                       if((event.type==KeyPress) || (event.type==ButtonPress) || (event.type==ButtonRelease) || (event.type==KeyRelease)) {
...
event_t gfx_wait()
{
	...
	while(1) {
		...
		if(event.type==KeyPress) {
			...
			myevent.key = XLookupKeysym(&event.xkey,0);
			return myevent;
		} else if(event.type==ButtonPress) {
			saved_xpos = event.xkey.x;
			saved_ypos = event.xkey.y;
			myevent.type = MOUSEDOWN;
			myevent.key = event.xbutton.button;
			return myevent;
